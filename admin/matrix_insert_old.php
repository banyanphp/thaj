...
echo "test";
 ...
 if($oper=="insertprice")  
	{ 		
		...
		echo $unitpricee= $tprice[0];
		echo $rowwiddd = $tprice[1];
		echo $prodqtty = $tprice[2];
		$totlprice = ($unitpricee) * ($prodqtty) ;	
		$martrixins= $_SESSION['matrixmailid'];																				
		$referencee = "trxn".'_'.$martrixins;
		
				$upqry = "Update $referencee SET fld_unitprice=$unitpricee,fld_totalprice=$totlprice where fld_id =$rowwiddd";
				mysql_query($upqry);			 				
