...
?>
<?php
//$query ="SELECT COUNT(*) AS id FROM tbl_postrequirement pst
...
//	$counter = mysql_query($query);
	//$num = mysql_fetch_array($counter,MYSQL_ASSOC)
	//$count = $num["id"];
	?>
...
<div id="loading_layer" style="display:none"><img src="img/ajax_loader.gif" alt="" /></div>                
  ...
  <form id="form1" name="form1" method="post">
            <div class="ui-widget">
          <!--io0  <input type="text" name="searchtxnid" id="tags" placeholder="Search for a Product" style="height:30px;Margin-bottom:-2px;margin-left:163px"/>
  ...
  <div id="full" style="display:block;width:100%;">
					<?php	
						$level2iddds = array();
						$resltsubcat ="Select distinct fld_level2id from tbl_level2 lvl2						
						where lvl2.fld_level1id in($catid)";
						$resltsubcatqry = mysql_query($resltsubcat) or die(mysql_error());
						while($rowsubcat = mysql_fetch_array($resltsubcatqry,MYSQL_ASSOC))
						{					
						...
						?>  
						<?php
						$reslttable ="Select distinct fld_tablename from tbl_level3 lvl3						
						where lvl3.fld_level2id in($lvl2idss) order by fld_tablename ASC";
						$reslttableqry = mysql_query($reslttable) or die(mysql_error());
						$ctablename= array();
						while($rowsubcat = mysql_fetch_array($reslttableqry,MYSQL_ASSOC))
						{					
				...
				$brdnqry1 = mysql_query($final);				
						...
						while($rowbrnd = mysql_fetch_array($brdnqry1,MYSQL_ASSOC))
						{					
						...
						<?php
						echo '<table cellpadding="6" cellspacing="6" border="1" rules="all" align="center" id="chktable" style="width:100%;">';
						$countproducts = count($producttss_array);
						$countbrnd = count($brandsss_array);
					  
						echo "<tr><td><b>Product/Brand</b></td>";
					
						foreach(array_unique($brand) as $key1=>$value1)
						{
							 echo "<td style='width:100px;'><b>" .$value1. "</b></td>"; 
						}
						echo '</tr>';
						//print_r($brand);
                                 ...
                                 <div class="accordion-group">
                                    ...
                                    <div class="accordion-group">
										...
										<ul class="nav nav-list">
                                            <li><a href="dashboard.php">Add a New Products</a></li>
                                        </ul>
