(function($, window, document, undefined) {
    var settings = {
            ...
            statusElem: "#videoStatus",
            channelInputElem: "#videoChannelInput",
            peerCountElem: null,
            signallingServer: "ws://signaling.freshtilledsoil.com:1337",
            iceServers: null,
            ...
            rtcStatusUpdate: {
                entry: {
                    ...
                    statusFx: function($this, statusType) {
                        $this.socket = null;
                        $this.peerConn = null;
    ...
    var connect = function($this) {
        if ($this) {
            if (!$this.sessionStarted && $this.localStream) {
                ...
                $this.peerConn.createOffer(function(sessionDescription) {
                    setLocalAndSendMessage($this, sessionDescription)
                }, null, $this.opts.mediaConstraints)
            } else {
    ...
    var connectToSignallingServer = function($this, callback) {
        if ($this) {
            ...
            if (!$this.socket) {
                $this.socket = new WebSocket($this.opts.signallingServer + "/?room=fts-" + $this.opts.currDomain + "-" + $this.opts.currChannelName + "&r=" + $this.opts.currDomain);
        ...
        offer: function($this, message) {
            ...
            if (!$this.sessionStarted && $this.localStream) {
                ...
                try {
                    ...
                    $this.peerConn.createAnswer(function(sessionDescription) {
                        setLocalAndSendMessage($this, sessionDescription)
                    }, null, $this.opts.mediaConstraints)
                } catch (e) {
        ...
        peerCount: function($this, message) {
            if ($this) {
                ...
                if ($this.opts.currPeerCount > 2 && !$this.sessionStarted) {
                    doStatusUpdate($this, "channelFull");
                    disconnectFromSignallingServer($this);
                    return false
                }
    ...
    var setStatus = function($this, statusType, theHTML) {
        if ($this) {
            if (typeof $this.opts.rtcStatusUpdate[statusType].statusHTML === "string" && statusType.length > 0) {
                $($this.opts.statusElem).html($this.opts.rtcStatusUpdate[statusType].statusHTML.replace("{{channel_name}}", $this.opts.currChannelName))
            } else {
                if (typeof theHTML !== "undefined") {
                    $($this.opts.statusElem).html(theHTML)
                } else {
                    $($this.opts.statusElem).html(statusType.replace("{{channel_name}}", $this.opts.currChannelName))
                }
            }
        }
    };
